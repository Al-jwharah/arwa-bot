...
export async function grokChat(opts: {
  ...
  const maxTokens = thinking === "fast" ? 280 : thinking === "deep" ? 700 : 420;
...
export async function grokImage(prompt: string, selfPortrait: boolean, look?: string): Promise<Buffer> {
  ...
  const full = selfPortrait
    ? `Photorealistic photograph of ${face}. Scene: ${prompt}. Natural light, fully clothed, modest contemporary clothes.`
    : prompt;
